...
import { getFramePower } from '@/lib/game/frame-powers';
...
export default function FramePowerButton({
  ...
  // If no user or no frame power, don't render anything
  ...
  const handleActivate = () => {
    ...
    // Trigger all effects associated with this frame power
  ...
  return (
    <div className="w-full flex justify-center mb-4">
      <motion.button
        whileTap={!hasUsed && !disabled ? { y: 2, scale: 0.95 } : {}}
      ...
      >
        ...
        <div className="relative z-10 w-8 h-8 flex items-center justify-center bg-white rounded-full shadow-sm">
          {framePower.id === 'crown' ? (
        ...
        <div className="relative z-10 flex flex-col items-start">
          <span className={`text-[12px] font-black tracking-wider uppercase leading-tight ${
            hasUsed ? 'text-[#7c7483]' : 'text-[#cba72f]'
          }`}>
            {framePower.name}
          </span>
          <span className={`text-[9px] font-bold ${
            hasUsed ? 'text-[#7c7483]/70' : 'text-[#1b1b1e]/70'
          }`}>
            {hasUsed ? 'DEJA ITILIZE' : '1 FWA POU CHAK JWÈT'}
          </span>
        </div>
      </motion.button>
    </div>

...
for (const item of data) {
    if (!item.fr && !translatedIds.has(item.ht.id)) {
        batch8.push(item.ht);
        if (batch8.length >= 50) break;
    }
}

fs.writeFileSync('scratch/ht_to_translate_batch8.json', JSON.stringify(batch8, null, 2));
console.log(`Created ht_to_translate_batch8.json with ${batch8.length} questions.`);

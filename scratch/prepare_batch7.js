...
for (const item of data) {
    if (!item.fr && !translatedIds.has(item.ht.id)) {
        batch7.push(item.ht);
        if (batch7.length >= 50) break;
    }
}

fs.writeFileSync('scratch/ht_to_translate_batch7.json', JSON.stringify(batch7, null, 2));
console.log(`Created ht_to_translate_batch7.json with ${batch7.length} questions.`);

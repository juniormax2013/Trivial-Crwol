...
import { jesus3DAnimationMap, type Jesus3DAction } from '@/src/config/jesus3DAnimations';

interface Jesus3DProps {
  action: Jesus3DAction;
  size?: number;
  className?: string;
  onActionComplete?: (action: Jesus3DAction) => void;
}
...
export default function Jesus3D({
  action,
  size = 280,
  className = '',
  onActionComplete,
  ...
  // Keep track of current model, mixer and active action
  ...
  const [renderAction, setRenderAction] = useState<Jesus3DAction>(action);
  ...
  useEffect(() => {
    ...
    camera.position.set(0, 1.15, 3.0); // Framed on body
    ...
    const hemiLight = new THREE.HemisphereLight('#ffffff', '#FFE088', 0.65);
    ...
    // Subtle golden/amber grid helper
    const grid = new THREE.GridHelper(10, 10, '#FFE088', '#fef3c7');
    grid.position.y = 0;
    ...
    const animate = () => {
      ...
      if (currentMixerRef.current) {
        ...
        // Loop clapping segment infinitely (1.0s to 2.7s) to avoid sit-down repeat
        if (currentActionRef.current && renderAction === 'clap') {
          if (currentActionRef.current.time >= 2.7) {
            currentActionRef.current.time = 1.0;
          }
        }
      }
  ...
  useEffect(() => {
    ...
    const glbUrl = jesus3DAnimationMap[renderAction];
    if (!glbUrl) return;

    const isLoop = renderAction === 'idle' || renderAction === 'clap';

    const setupModel = (model: THREE.Group, animations: THREE.AnimationClip[]) => {
      console.log("Loaded jesus animation:", renderAction, animations.length, animations.map(a => a.name));
      ...
      if (animations && animations.length > 0) {
        ...
        currentActionRef.current = animAction;
        ...
        // Slow down idle animation for a calmer feeling
        ...
        // If it is the clapping animation, loop only the standing & clapping part (1.0s to 2.7s) to avoid sitting down loop
        if (renderAction === 'clap') {
    ...
    if (glbCache[glbUrl]) {
      ...
      loader.load(
        ...
        undefined,
        (err) => {
          console.error(`Error al cargar animación 3D de Jesús: ${renderAction}:`, err);
          setLoading(false);
    ...
    >
      {loading && (
        <div className="absolute inset-0 z-20 flex items-center justify-center bg-transparent pointer-events-none">
          <div className="w-6 h-6 border-2 border-[#FFE088] border-t-transparent rounded-full animate-spin" />
        </div>
      )}
      {error && (
